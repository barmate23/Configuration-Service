...
//import com.stockmanagementsystem.request.PackingProfileLevelRequest;
...
//                    // =================================================
//                    PackingProfileConfigMaster profile = new PackingProfileConfigMaster();
//                    profile.setOrganizationId(loginUser.getOrgId());
//                    profile.setSubOrganizationId(loginUser.getSubOrgId());
//                    profile.setDescription("Uploaded from template");
//                    profile.setItem(item);
//                    profile.setSupplier(sim.getSupplier());
...
//
//                    // --- PRIMARY LEVEL ---
//                    PackingProfileLevel primaryLevel = new PackingProfileLevel();
//                    primaryLevel.setProfile(profile);
//                    primaryLevel.setLevelOrder(1);
//                    primaryLevel.setUnitsPerParent(primaryUnits);
//                    primaryLevel.setPackagingMaster(primaryPackaging);
//                    primaryLevel.setHierarchyLevel(hierarchyMap.get("PRIMARY")); // assuming PRIMARY exists
//                    primaryLevel.setCreatedBy(loginUser.getUserId());
//                    primaryLevel.setCreatedOn(new Date());
//                    packingProfileLevelRepository.save(primaryLevel);
//
//                    // --- SECONDARY LEVEL ---
//                    if (secondaryPackaging != null) {
//                        PackingProfileLevel secLevel = new PackingProfileLevel();
//                        secLevel.setProfile(profile);
//                        secLevel.setLevelOrder(2);
//                        secLevel.setUnitsPerParent(secondaryUnits);
//                        secLevel.setPackagingMaster(secondaryPackaging);
//                        secLevel.setHierarchyLevel(hierarchyMap.get("SECONDARY"));
//                        secLevel.setCreatedBy(loginUser.getUserId());
//                        secLevel.setCreatedOn(new Date());
//                        packingProfileLevelRepository.save(secLevel);
//                    }
//
//                    // --- TERTIARY LEVEL ---
//                    if (tertiaryPackaging != null) {
//                        PackingProfileLevel terLevel = new PackingProfileLevel();
//                        terLevel.setProfile(profile);
//                        terLevel.setLevelOrder(3);
//                        terLevel.setUnitsPerParent(tertiaryUnits);
//                        terLevel.setPackagingMaster(tertiaryPackaging);
//                        terLevel.setHierarchyLevel(hierarchyMap.get("TERTIARY"));
//                        terLevel.setCreatedBy(loginUser.getUserId());
//                        terLevel.setCreatedOn(new Date());
//                        packingProfileLevelRepository.save(terLevel);
//                    }
//
//                    // =================================================
//                    // ITEM–SUPPLIER MAPPING
//                    // =================================================
//                    ItemSupplierPackingProfileMap mapping = buildItemSupplierPackingProfileMap(
//                                    item,
//                                    sim.getSupplier(),
//                                    profile
//                            );
...
//            PackingProfileConfigMaster profile =
//                    packingProfileRepo
//                            .findById(request.getPackingProfileId())
//                            .filter(p ->
//                                    !p.getIsDeleted()
...
//    @Override
//    @Transactional
//    public BaseResponse<PackingProfileConfigMaster> savePackingProfileConfig(PackingProfileConfigRequest request) {
//        String logId = loginUser.getLogId();
//        BaseResponse<PackingProfileConfigMaster> response = new BaseResponse<>();
//
//        try {
//            PackingProfileConfigMaster master;
//            if (request.getId() != null) {
//                master = packingProfileRepo.findById(request.getId())
//                        .orElseThrow(() -> new IllegalArgumentException("Packing profile not found with ID: " + request.getId()));
//                master.setModifiedBy(loginUser.getUserId());
//                master.setModifiedOn(new Date());
//            } else {
//                master = new PackingProfileConfigMaster();
//                master.setOrganizationId(loginUser.getOrgId());
//                master.setSubOrganizationId(loginUser.getSubOrgId());
//                master.setCreatedBy(loginUser.getUserId());
//                master.setCreatedOn(new Date());
//            }
//
//            master.setDescription(request.getDescription());
//            master.setMoqLevel(request.getMoqLevel());
//            master.setMoqQty(request.getMoqQty());
//
...
//            }
//
//            PackingProfileConfigMaster savedMaster = packingProfileRepo.save(master);
//
//            // Handle levels
//            if (request.getLevels() != null && !request.getLevels().isEmpty()) {
//                // If updating, ideally we should delete old levels or match them
//                // For simplicity here, if it's a new master or complete replacement:
//                if (request.getId() != null) {
//                    // Logic to handle existing levels (could delete all then re-add if that's the requirement)
//                }
//
//                for (PackingProfileLevelRequest levelReq : request.getLevels()) {
//                    PackingProfileLevel level = new PackingProfileLevel();
//                    level.setProfile(savedMaster);
//                    level.setLevelOrder(levelReq.getLevelOrder());
//                    level.setUnitsPerParent(levelReq.getUnitsPerParent());
//
//                    if (levelReq.getHierarchyLevelId() != null) {
//                        PackingHierarchyLevel hierarchy = packingHierarchyLevelRepository.findById(levelReq.getHierarchyLevelId())
//                                .orElseThrow(() -> new IllegalArgumentException("Hierarchy level not found with ID: " + levelReq.getHierarchyLevelId()));
//                        level.setHierarchyLevel(hierarchy);
//                    }
//
//                    if (levelReq.getPackagingId() != null) {
//                        PackagingMaster packaging = packagingMasterRepository.findById(levelReq.getPackagingId())
//                                .orElseThrow(() -> new IllegalArgumentException("Packaging master not found with ID: " + levelReq.getPackagingId()));
//                        level.setPackagingMaster(packaging);
//                    }
//
//                    level.setCreatedBy(loginUser.getUserId());
//                    level.setCreatedOn(new Date());
//                    packingProfileLevelRepository.save(level);
//                }
//            }
//
//            response.setStatus(1);
//            response.setCode(200);
//            response.setMessage("Packing profile configuration saved successfully");
//            response.setData(Collections.singletonList(savedMaster));
//
//        } catch (Exception ex) {
//            log.error("{} | savePackingProfileConfig | FAILED", logId, ex);
//            response.setStatus(0);
//            response.setCode(500);
//            response.setMessage("Failed to save packing profile: " + ex.getMessage());
//        }
//
//        return response;
//    }
//    @Override
//    public BaseResponse<PackingProfileResponse> getPackingProfileByIdProperly(Long id) {
//        String logId = loginUser.getLogId();
//        BaseResponse<PackingProfileResponse> response = new BaseResponse<>();
//
//        try {
//            PackingProfileConfigMaster master = packingProfileRepo.findById(id)
//                    .orElseThrow(() -> new IllegalArgumentException("Packing profile not found"));
//
//            PackingProfileResponse profileRes = new PackingProfileResponse();
//            profileRes.setId(master.getId());
//            profileRes.setDescription(master.getDescription());
//            profileRes.setItemCode(master.getItem() != null ? master.getItem().getItemCode() : null);
//            profileRes.setItemName(master.getItem() != null ? master.getItem().getName() : null);
//            profileRes.setSupplierCode(master.getSupplier() != null ? master.getSupplier().getErpSupplierId() : null);
//            profileRes.setSupplierName(master.getSupplier() != null ? master.getSupplier().getSupplierName() : null);
//            profileRes.setMoqLevel(master.getMoqLevel());
//            profileRes.setMoqQty(master.getMoqQty());
//            profileRes.setIsActive(master.getIsActive());
////
////            List<PackingProfileLevelResponse> levels = master.getLevels().stream().map(lvl -> {
////                PackingProfileLevelResponse lvlRes = new PackingProfileLevelResponse();
////                lvlRes.setId(lvl.getId());
////                lvlRes.setLevelOrder(lvl.getLevelOrder());
////                lvlRes.setUnitsPerParent(lvl.getUnitsPerParent());
////                lvlRes.setLevelCode(lvl.getHierarchyLevel() != null ? lvl.getHierarchyLevel().getLevelCode() : null);
////                lvlRes.setPackagingCode(lvl.getPackagingMaster() != null ? lvl.getPackagingMaster().getPackagingCode() : null);
////                lvlRes.setPackagingName(lvl.getPackagingMaster() != null ? lvl.getPackagingMaster().getPackagingName() : null);
////                return lvlRes;
////            }).collect(Collectors.toList());
////
////            profileRes.setLevels(levels);
//
//            response.setStatus(1);
//            response.setCode(200);
//            response.setData(Collections.singletonList(profileRes));
//
//        } catch (Exception ex) {
//            log.error("{} | getPackingProfileByIdProperly | FAILED", logId, ex);
//            response.setStatus(0);
//            response.setCode(500);
//            response.setMessage("Failed to fetch packing profile: " + ex.getMessage());
//        }
//        return response;
//    }
//}
